    ...
    <div class="mb-4">
        <div class="flex items-center">
            <form class="flex items-center w-full" @submit.prevent="searching">
                ...
                <button @click="role_id = 0" v-if="can_create"
            ...
            <tbody>
                <tr v-for="(item) in roles" :key="item.id"
                    ...
                    <td class="py-4 px-6 whitespace-nowrap text-center">
                        <button type="button" @click="role_id = item.id" v-if="item.can_update"
                        ...
                        <button type="button" @click="deleteRole(item.id)" v-if="item.can_delete"
...
const { can_create, role_id, search, roles, getRoles, destroyRole } = useRole();
...
const deleteRole = async (id: number) => {
    ...
    if (willDelete) {
        let response = await destroyRole(id);
        if ( response ) {
